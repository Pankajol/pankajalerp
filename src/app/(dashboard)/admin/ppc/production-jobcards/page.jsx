...
export default function ProductionJobCardList() {
  ...
  const fetchData = async () => {
    ...
    try {
      ...
      const res = await axios.get(`/api/ppc/production-jobcards?${params.toString()}`, {
      ...
      setKpi({
        total: cards.length,
        inProgress: cards.filter(c => !["Completed","Ready","Delivered"].includes(c.status)).length,
  ...
  const KPICards = [
    ...
    { label: "Completed", value: kpi.completed, icon: FaCheck, color: "bg-green-500" },
    ...
    <div className="min-h-screen bg-gray-50 py-8 px-4 sm:px-10">
      <div className="max-w-7xl mx-auto">
        <h1 className="text-2xl font-extrabold text-gray-900 mb-6 flex items-center gap-3">
          <FaIndustry className="text-indigo-600" /> Production Job Cards
        </h1>
        {/* KPI */}
        ...
        <div className="bg-white rounded-2xl shadow-sm border p-4 mb-6 flex flex-col sm:flex-row gap-4 items-center justify-between">
          <div className="relative flex-1">
            ...
            <input
              ...
              placeholder="Search by job card no, item name..."
              value={search}
          ...
          >
            ...
            {["Planned","In Progress","QC","Completed","Ready","Delivered"].map(s => (
              <option key={s} value={s}>{s}</option>
            ))}
          </select>
          <button
            onClick={() => router.push("/admin/ppc/production-jobcards/new")}
          ...
          <table className="w-full text-sm">
            <thead className="bg-gray-50 border-b">
              <tr>
                ...
                <th className="px-6 py-4 text-left text-xs font-bold uppercase text-gray-400">Item</th>
                <th className="px-6 py-4 text-left text-xs font-bold uppercase text-gray-400">Order</th>
              ...
              {loading ? (
                ...
                jobCards.map(jc => (
                  <tr key={jc._id} className="hover:bg-indigo-50/20">
                    ...
                    <td className="px-6 py-4">{jc.itemName}</td>
                    <td className="px-6 py-4">{jc.productionOrder?.orderNumber || "N/A"}</td>
                    <td className="px-6 py-4 text-center">{jc.quantity}</td>
                    <td className="px-6 py-4 text-center">{statusBadge(jc.status)}</td>
                    <td className="px-6 py-4 text-center">
                      <button
                        onClick={() => router.push(`/admin/ppc/production-jobcards/${jc._id}`)}
