...
const handleSave = async () => {
  ...
  try {
    ...
    await fetchAllBills();
    router.push(`/construction/progress-billing?workOrderId=${workOrder._id}&billId=${res.data.data._id}`);
    const billsRes = await api.get(`/construction/progress-billing?workOrderId=${workOrder._id}`, headers);
  ...
  const handleEditBill = (billId) => {
    setEditingBill(true);
    router.push(`/construction/progress-billing?billId=${billId}`);
    setViewMode("create");
  };
  ...
  const recordPayment = async () => {
    ...
    if (amount > bill.remainingAmount) {
      toast.error(`Amount cannot exceed remaining balance (${formatCurrency(bill.remainingAmount)})`);
      return;
    }
  ...
  const getPaymentProgress = () => {
    if (!bill) return 0;
    const totalBill = bill.items?.reduce((s, i) => s + (i.amount || 0), 0) || 0;
    ...
    <div className="min-h-screen bg-gray-100 py-6 px-4 sm:px-6 lg:px-8">
              ...
              <table className="w-full text-sm border-collapse">
                <thead>
                  <tr className="bg-gray-50 border-b border-gray-200">
                    ...
                    <th className="px-6 py-3 text-left text-[10px] font-bold uppercase tracking-wider text-gray-500">Type</th>
              ...
              <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
                <div>
                  ...
                  <Select
                    ...
                    isClearable
                    isSearchable
                    isDisabled={!!initialBillId} // Disable if editing a bill
                    styles={{
                      control: (base) => ({
                        ...base,
                        borderColor: '#e2e8f0',
      ...
      <style jsx global>{`
        @media print {
          ...
          #print-area { position: absolute; left: 0; top: 0; width: 100%; background: white; padding: 40px; }
