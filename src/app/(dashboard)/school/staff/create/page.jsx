...
export default function CreateStaff() {
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      ...
      await api.post("/school/staff", formData, headers);
      toast.success("🎉 Staff member created successfully!");
      setTimeout(() => router.push("/school/staff"), 1500);
    } catch (err) {
          ...
          <form onSubmit={handleSubmit} className="relative">
                ...
                <motion.div key="step0" initial={{ opacity: 0, x: 30 }} animate={{ opacity: 1, x: 0 }} exit={{ opacity: 0, x: -30 }} className="p-8">
                  <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
                    <div className="space-y-1">
                      <label className="block text-sm font-semibold text-gray-700">Staff ID <span className="text-red-500">*</span></label>
                      <input type="text" name="staffId" value={formData.staffId} onChange={handleChange} className={`w-full border ${errors.staffId ? "border-red-500" : "border-gray-200"} rounded-2xl px-5 py-3 focus:border-violet-500 focus:ring-2 focus:ring-violet-200 outline-none transition-all`} required />
                    ...
                    <div className="space-y-1">
                      <label className="block text-sm font-semibold text-gray-700">Login Password</label>
                      <input type="password" name="password" value={formData.password} onChange={handleChange} className="w-full border border-gray-200 rounded-2xl px-5 py-3 focus:border-violet-500 focus:ring-2 focus:ring-violet-200 outline-none transition-all" placeholder="Leave blank to generate default" minLength={6} />
                    </div>
                    <div className="space-y-1">
              ...
              {currentStep === 1 && (
                <motion.div key="step1" initial={{ opacity: 0, x: 30 }} animate={{ opacity: 1, x: 0 }} exit={{ opacity: 0, x: -30 }} className="p-8">
                  ...
                  {formData.qualifications.length === 0 && (
                    <p className="text-gray-400 italic text-center py-8">No qualifications added yet.</p>
                  )}
              ...
              {currentStep === 2 && (
                <motion.div key="step2" initial={{ opacity: 0, x: 30 }} animate={{ opacity: 1, x: 0 }} exit={{ opacity: 0, x: -30 }} className="p-8">
                  ...
                  {formData.workExperience.length === 0 && (
                    <p className="text-gray-400 italic text-center py-8">No work experience added yet.</p>
                  )}
                ...
                <motion.div key="step3" initial={{ opacity: 0, x: 30 }} animate={{ opacity: 1, x: 0 }} exit={{ opacity: 0, x: -30 }} className="p-8">
                  <div className="grid grid-cols-1 gap-6">
                    <div className="space-y-1">
                      ...
                      <input type="text" name="address" value={formData.address} onChange={handleChange} className="w-full border border-gray-200 rounded-2xl px-5 py-3 focus:border-violet-500 focus:ring-2 focus:ring-violet-200 outline-none transition-all" placeholder="123 Main St, City" />
                    </div>
            ...
            <div className="border-t border-gray-100 px-8 py-6 flex justify-between items-center bg-gray-50">
              ...
              {currentStep === steps.length - 1 ? (
                <button type="button" disabled={loading} className="flex items-center gap-3 px-10 py-3.5 bg-gradient-to-r from-violet-600 to-indigo-600 text-white rounded-2xl font-semibold shadow-lg shadow-violet-500/30 hover:shadow-xl transition-all disabled:opacity-70 active:scale-[0.97]">
                  <FaSave /> {loading ? "Creating..." : "Create Staff Member"}
                </button>
              ) : (
