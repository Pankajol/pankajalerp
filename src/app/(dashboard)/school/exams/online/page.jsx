...
export default function OnlineExamsPage() {
  ...
  const fetchExams = async (page = 1) => {
    ...
    try {
      ...
      const params = { page, limit: 10, search, type: "online" };
  ...
  const handleDelete = async (id) => {
    if (!confirm("Delete this online exam?")) return;
    try {
      const token = localStorage.getItem("token");
      const headers = { headers: { Authorization: `Bearer ${token}` } };
      ...
      <div className="flex items-center justify-between mb-6 flex-wrap gap-4">
        <div>
          <h2 className="text-2xl font-bold text-gray-800 flex items-center gap-2">
            <FaLaptop className="text-indigo-500" size={24} />
            Online Exams
          </h2>
        ...
        <motion.button
          ...
          whileTap={{ scale: 0.95 }}
          onClick={() => router.push("/school/exams/online/create")}
          ...
          <table className="w-full text-sm">
            <thead>
              <tr className="bg-gradient-to-r from-indigo-50 to-indigo-100/50">
                ...
                <th className="px-6 py-4 text-center text-xs font-bold uppercase tracking-wider text-indigo-700">Time</th>
                  ...
                  <td colSpan="8" className="py-16 text-center">
                    <div className="flex flex-col items-center">
                      <div className="w-20 h-20 bg-indigo-50 rounded-full flex items-center justify-center mb-4">
                        <FaLaptop size={32} className="text-indigo-300" />
                      </div>
                      <h3 className="text-lg font-semibold text-gray-700">No online exams found</h3>
                ...
                exams.map((exam, idx) => (
                  ...
                  >
                    ...
                    <td className="px-6 py-4 text-right flex items-center justify-end gap-1">
                      <button
                        onClick={() => router.push(`/school/exams/online/${exam._id}`)}
