...
export default function AttendancePage() {
  ...
  const fetchSessions = async () => {
    try {
      ...
      const res = await api.get("/school/training/sessions", {
  ...
  const fetchAttendance = useCallback(async (page = 1) => {
    ...
    try {
      ...
      const params = { page, limit: 15, search, session: sessionFilter, status: statusFilter };
      const res = await api.get("/school/training/attendance", { params, ...headers });
  ...
  const handleDelete = async () => {
    ...
    try {
      const token = localStorage.getItem("token");
      await api.delete(`/school/training/attendance/${deleteId}`, {
        headers: { Authorization: `Bearer ${token}` },
      });
  ...
  const getStatusConfig = (status) => {
    const configs = {
      present: { icon: <FaUserCheck className="text-green-500" />, color: "bg-green-100 text-green-700" },
      absent: { icon: <FaUserTimes className="text-red-500" />, color: "bg-red-100 text-red-700" },
      late: { icon: <FaUserClock className="text-yellow-500" />, color: "bg-yellow-100 text-yellow-700" },
    };
    ...
    <div className="min-h-screen bg-gradient-to-br from-slate-50 via-indigo-50 to-white p-6">
        ...
        <div className="flex flex-wrap items-center justify-between gap-4 mb-8">
          <div className="flex items-center gap-4">
            ...
            <div>
              ...
              <p className="text-gray-500 mt-1">Track teacher attendance in sessions</p>
            </div>
          </div>
          <motion.button
            whileHover={{ scale: 1.05 }}
            whileTap={{ scale: 0.95 }}
            onClick={() => router.push("/school/training/attendance/create")}
                        ...
                        >
                          ...
                          <td className="px-8 py-5 text-right">
                            <div className="flex items-center justify-end gap-2 opacity-70 group-hover:opacity-100 transition-all">
                              ...
                              <button
                                onClick={() => router.push(`/school/training/attendance/${r._id}/edit`)}
          ...
          {meta.pages > 1 && (
            <div className="px-8 py-5 border-t flex items-center justify-between bg-gray-50">
              ...
              <div className="flex gap-2">
                <button
                  onClick={() => fetchAttendance(meta.page - 1)}
      ...
      {showDeleteModal && (
        <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/50 backdrop-blur-sm">
          ...
          >
            <h3 className="text-2xl font-bold text-gray-900 mb-2">Delete Attendance Record?</h3>
