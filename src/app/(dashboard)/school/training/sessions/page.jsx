...
export default function SessionsPage() {
  ...
  const fetchSessions = useCallback(async (page = 1) => {
    ...
    try {
      ...
      const params = { page, limit: 15, search, program: programFilter };
      const res = await api.get("/school/training/sessions", { params, ...headers });
  ...
  const handleDelete = async () => {
    ...
    try {
      const token = localStorage.getItem("token");
      await api.delete(`/school/training/sessions/${deleteId}`, {
        headers: { Authorization: `Bearer ${token}` },
      });
      toast.success("Session deleted");
      setShowDeleteModal(false);
      setDeleteId(null);
      fetchSessions(meta.page);
    } catch (err) {
    ...
    <div className="min-h-screen bg-gradient-to-br from-slate-50 via-indigo-50 to-white p-6">
      <div className="max-w-7xl mx-auto">
        ...
        <div className="flex flex-wrap items-center justify-between gap-4 mb-8">
          <div className="flex items-center gap-4">
            ...
            <div>
              <h1 className="text-4xl font-bold text-gray-900">Training Sessions</h1>
              <p className="text-gray-500 mt-1">Manage program sessions</p>
            </div>
          </div>
          <motion.button
            whileHover={{ scale: 1.05 }}
            whileTap={{ scale: 0.95 }}
            onClick={() => router.push("/school/training/sessions/create")}
                ...
                <AnimatePresence>
                  {loading ? (
                    ...
                    sessions.map((s, index) => (
                      ...
                      >
                        <td className="px-8 py-5 font-medium text-gray-900">{s.title}</td>
                        <td className="px-8 py-5 text-gray-600">{s.program?.title || "—"}</td>
                        <td className="px-8 py-5 text-gray-600">
                          <div className="flex flex-col">
                            ...
                            {s.startTime && (
                              <span className="flex items-center gap-1.5 text-sm text-gray-400 mt-0.5">
                                <FaClock size={12} />
                        ...
                        <td className="px-8 py-5 text-right">
                          <div className="flex items-center justify-end gap-2 opacity-70 group-hover:opacity-100 transition-all">
                            <button
                              onClick={() => router.push(`/school/training/sessions/${s._id}`)}
                            ...
                            <button
                              onClick={() => router.push(`/school/training/sessions/${s._id}/edit`)}
      ...
      {showDeleteModal && (
        <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/50 backdrop-blur-sm">
          ...
          >
            <h3 className="text-2xl font-bold text-gray-900 mb-2">Delete Session?</h3>
            <p className="text-gray-500 mb-6">
              This action cannot be undone. All attendance records for this session will also be deleted.
            </p>
